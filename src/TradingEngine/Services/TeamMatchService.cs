...
public class TeamMatchService(
    ...
    ILogger<TeamMatchService> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                var sportEvents = await sportEventCatalogue.GetAllAsync(cancellationToken);
                var oddsEvents = await oddsEventCatalogue.GetAllAsync(cancellationToken);
